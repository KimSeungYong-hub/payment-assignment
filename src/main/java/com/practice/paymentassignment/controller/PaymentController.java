...
public class PaymentController {
    ...
    @IdempotentRequest(ttl = 300, prefix = "idempotency:readyPayment:")
    ...
    public ResponseEntity<PaymentDto.Prepare.Response> createPayment(@RequestBody PaymentDto.Prepare.Request request,
                                                                    @RequestHeader(value = "Idempotency-Key") String idempotencyKey) {
        PaymentDto.Prepare.Response response = paymentUseCase.createPayment(request, idempotencyKey);
        return ResponseEntity.ok(response);
    }

    @IdempotentRequest(ttl = 300, prefix = "idempotency:confirmPayment:")
    @PostMapping("/payments/{paymentId}/approvals") // 마찬가지
    public ResponseEntity<PaymentDto.Approve.Response> confirmPayment(@PathVariable("paymentId") Long paymentId, @RequestBody PaymentDto.Approve.Request request) {
        Long userId = 1L;
